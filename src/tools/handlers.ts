...
export class ToolHandlers {
  ...
  async getTask(args: any): Promise<any> {
    try {
      const sanitizedArgs = sanitizeInput(args);
  ...
  async getWorkspaces(args: any): Promise<any> {
    try {
      const sanitizedArgs = sanitizeInput(args);
  ...
  async getSpaces(args: any): Promise<any> {
    try {
      const sanitizedArgs = sanitizeInput(args);
        ...
        content: [
          {
            ...
            text: `Spaces:\n\n${result.spaces.map(space => 
              `• ${space.name} (ID: ${space.id})\n  Private: ${space.private}\n  Statuses: ${space.statuses?.length || 0}`
            ).join('\n\n')}`
          }
        ]
      };
  ...
  async getAuthorizedUser(args: any): Promise<any> {
    try {
      const sanitizedArgs = sanitizeInput(args);
